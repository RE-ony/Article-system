...
if ($hespon_id=="0") {
	...
	$result=mysqli_query($link,"SELECT articles.article_id,articles.article_title,articles.article_content,articles.author,articles.time,arcticle_title.file_all_path FROM articles LEFT JOIN arcticle_title on articles.article_id=arcticle_title.file_id WHERE articles.author='$name' ORDER BY articles.article_id ASC LIMIT $start,10;");
	$count=mysqli_num_rows($result);
	$data=array();
	if($count>0){
		for($i=0;$i<$count;$i++){
